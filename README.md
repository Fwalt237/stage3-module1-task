...
- [x] _authorId_ should be mapped to the dataSourceAuthor datasource.
...
For example: dataSourceAuthor.txt, content.txt which should contain the required data.
...
- [x] Get News by id operation – return object by provided dataSourceModel id.
- [x] For update operation – update only title, content, authorId by provided dataSourceModel id and return updated object.
- [x] Delete News operation – delete entity from list objects by provided dataSourceModel id and return boolean value.
...
5. All the dataSourceModel data should be generated automatically in the module-repository with the amount of 20 and stored in array (database emulation) when the application is running.
   Reading default data for dataSourceModel generating should be from resources package in dataSourceAuthor.txt, content.txt and dataSourceModel.txt files.
...
The finished sample application can be found in the [__solution__](./solution/dataSourceModel-management.jar) package at the root of the project, which can be run with the following command:
``` java
   java -jar dataSourceModel-management.jar
```
